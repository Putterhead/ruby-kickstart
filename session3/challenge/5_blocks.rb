...
def spiral_access(arrays, iteration=0, &block)
  ...
  return if y_max/2 < iteration || x_max/2 < iteration # base step

  iteration.upto x_max-iteration do |x| # top row
    block.call arrays[iteration][x]
  end

  (iteration + 1).upto y_max-iteration do |y| # right column
    block.call arrays[y][x_max-iteration]
  end

  (x_max - 1 - iteration).downto iteration do |x| # bottom ro
    block.call arrays[y_max-iteration][x]
  end 

  (y_max - 1 - iteration).downto iteration+1 do |y| # left column
    block.call arrays[y][iteration]
  end

  sprial_access arrays, iteration+1, &block # recursive step
end
